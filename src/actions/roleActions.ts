...
const createRole = async (name: string, permissionIds: number[]) => {
  try {
    if (!permissionIds.length) {
      createLogger().error("No Permission selected");
    ...
    if (existingPermissions.length !== permissionIds.length) {
      createLogger().error("Some permissions do not exist.");
    ...
    createLogger().error(`Error creating role`);
...
const getAllRoles = async () => {
  try {
    ...
    createLogger().error(`Error fetching roles`);
...
const getRoleById = async (id: number) => {
  try {
    ...
    createLogger().error(`Error fetching role`);
...
const updateRole = async (
  ...
  try {
    ...
    createLogger().error(`Error updating role`);
...
const deleteRole = async (id: number) => {
  try {
    ...
    createLogger().error(`Error deleting role`);
